...
program define plotcoeffs
    ...
    * Allows time series operators 
    syntax [anything], [estimates(string) b(string) se(string) graphs(string) label(string) cumulative(string) lcolor(string) fcolor(string) yshift(string) savedata(string asis) combine *]
    ...
    * * output error if the graphs string has incorrect number of words
    ...
    else if ((( `n_est' != `n_gr' & `n_est' >= 2) | ( `n_gr' >= 2 & `n_est' <= 1)) & "`combine'" != "combine") {
    ...
    * * localize graph commands per set of estimation results
    ...
    foreach V in `estimates' {
        ...
        * * * store graph commands
        if ( "`type'" == "bar") {
            local graph_`V' "(bar b_`V' order, color(`lcolor') fintensity(inten100) barwidth(.5))"
        }
        
        if ( "`type'" == "err" | "`type'" == "") {
            local graph_`V' "(scatter b_`V' order, mlcolor(`lcolor') mfcolor(`fcolor') mstyle(p1)) (rcap u_`V' l_`V' order, lcolor(`lcolor'))"
        }
        
        if ( "`type'" == "line") {
            local graph_`V' "(line b_`V' order, lcolor(`lcolor')) (line u_`V' order, lpattern(dash) lcolor(`color1')) (line l_`V' order, lpattern(dash) lcolor(`lcolor'))"
        }
        
        if ( "`type'" == "linearea") {
            local graph_`V' "(rarea u_`V' l_`V' order, color(`lcolor'*.5)) (line b_`V' order, lcolor(`lcolor'))"
        }
        
        if ( "`type'" == "linenose") {
            local graph_`V' "(line b_`V' order, lpattern(dash) lcolor(`lcolor'))"
        }

        if ( "`type'" == "nose") {
            local graph_`V' "(scatter b_`V' order, mlcolor(`lcolor') mfcolor(`fcolor'))"
        }
        
        if ("`type'" == "connect") {
            local graph_`V' "(connected b_`V' order, lcolor(`lcolor') mlcolor(`lcolor') mfcolor(`fcolor') msymbol(circle) lpattern(solid))"
        }
