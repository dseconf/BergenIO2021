...
if loadz ;
  ...
  */
  ...
  @ Makes instruments and variables consistent with BLP (1995) specification @
  x = const~(hpwt)~space~air~mpd;
  w = const~ln(hpwt)~air~ln(mpg)~ln(space)~trend;
  {xexch,xtot} = exch(x) ;  @ xexch is sum(ownx), xtot is sum(allx) @
  {wexch,wtot} = exch(w) ;
  xexch = xexch-x;
  wexch = wexch-w;
...
format /ma1/rdn 16,12;
outwidth 256;

outfile = "../output/residuals.csv";
